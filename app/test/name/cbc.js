...
export default class CBC extends Component {
  ...
  componentDidMount() {
    ...
    firebaseApp.database().ref('cbc').orderByChild('createdAt').limitToLast(this.state.counter).on('value',
    (snapshot) => {
    ...
    return (
      <View style={styles.container}>
        <Header backgroundColor='white'
          leftComponent={{ icon: 'arrow-back', color: 'red' ,onPress: () => Actions.name(),}}
          centerComponent={{ text: 'CBC', style: { color: 'red' } }}
        />
        <ListView
  ...
  _onEndReached = () => {
    ...
    if (!this.state.isEmpty && !this.state.isFinished && !this.state.isLoading) {
      ...
      firebaseApp.database().ref('cbc').off()
      firebaseApp.database().ref('cbc').orderByChild('createdAt').limitToLast(this.state.counter+1).on('value',
      (snapshot) => {
