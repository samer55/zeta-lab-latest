...
export default class Balqa extends Component {
  constructor(props) {
    super(props)
  ...
  componentDidMount() {
    ...
    firebaseApp.database().ref('balqa').orderByChild('createdAt').limitToLast(this.state.counter).on('value',
    (snapshot) => {
      ...
      <ImageBackground
        ...
        <View
          style={{width:screenWidth,height:screen}} >
            ...
            <Text style={{fontSize:30,color:'black',fontWeight:'bold',selfAlign:'center',textAlign:'center'}}>ابحث عن مختبرك</Text>
            <ListView
  ...
  _renderRow = (data) => {
    ...
    return (
      ...
      >
        ...
        <Text style={{margin:3,fontSize:19,fontWeight:'bold',color:'#000000',fontFamily:'Roboto',selfAlign:'center',textAlign:'center'}}>{data.username}</Text>

        <View style={{flexDirection:'row'}}>

<Icon name='map-marker' size={15} color='grey'/>
<Text style={{margin:3,fontSize:15,fontWeight:'bold',color:'#09090a',fontFamily:'Roboto',selfAlign:'center'}}>{data.title}</Text>

</View>
          <TouchableOpacity  onPress={()=> this.user(data)}>
          <Text style={{fontSize:20,fontWeight:'bold',color:'#F44E3B'}}>شاهد ملف المختبر</Text>
          </TouchableOpacity>
  ...
  _onEndReached = () => {
    ...
    if (!this.state.isEmpty && !this.state.isFinished && !this.state.isLoading) {
      ...
      firebaseApp.database().ref('balqa').off()
      firebaseApp.database().ref('balqa').orderByChild('createdAt').limitToLast(this.state.counter+1).on('value',
      (snapshot) => {
