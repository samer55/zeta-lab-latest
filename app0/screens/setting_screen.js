  ...
  _handleSave = () => {
        ...
        .then((snapshot) => {
          if (snapshot.val()) {
            ...
            .then(() => {
              ...
              firebaseApp.database().ref('us').child(this.props.appStore.user.uid).update({username:this.state.name})
    ...
    if (this.state.password) {
      if (this.state.password.length < 6) {
        ...
        firebaseApp.database().ref('us').child(this.props.appStore.user.uid).update({pass:this.state.password})
