...
export default class Testp extends Component {
  ...
  componentDidMount() {
    ...
    firebaseApp.database().ref('testp').orderByChild('createdAt').limitToLast(this.state.counter).on('value',
    (snapshot) => {
  ...
  _onEndReached = () => {
    ...
    if (!this.state.isEmpty && !this.state.isFinished && !this.state.isLoading) {
      ...
      firebaseApp.database().ref('testp').off()
      firebaseApp.database().ref('testp').orderByChild('createdAt').limitToLast(this.state.counter+1).on('value',
      (snapshot) => {
