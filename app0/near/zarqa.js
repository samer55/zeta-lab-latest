...
export default class Zarqa extends Component {
  constructor(props) {
    super(props)
  ...
  componentDidMount() {
    ...
    firebaseApp.database().ref('zarqa').orderByChild('createdAt').limitToLast(this.state.counter).on('value',
    (snapshot) => {
...
<Text style={{margin:3,fontSize:15,fontWeight:'bold',color:'#09090a',fontFamily:'Roboto',selfAlign:'center'}}>{data.title}</Text>
  ...
  _onEndReached = () => {
    ...
    if (!this.state.isEmpty && !this.state.isFinished && !this.state.isLoading) {
      ...
      firebaseApp.database().ref('zarqa').off()
      firebaseApp.database().ref('zarqa').orderByChild('createdAt').limitToLast(this.state.counter+1).on('value',
      (snapshot) => {
