...
user:'',
  ...
  filter = search => {
    ...
    const filteredData = this.state.dataSource.filter(a =>
      a.names.toLowerCase().includes(value)
    );
  ...
  SearchFilterFunction(text){
       ...
       const newData = this.arrayholder.filter(function(item){
           const itemData = item.names.toUpperCase()
    ...
    componentDidMount() {
      const commentsRef = firebaseApp.database().ref('us');
      ...
      <List>
          ...
          style={{width:80,height:80}}
           />
              ...
              <Body>
                <Text style={{fontWeight:'800',fontSize:20}}>{data.names}</Text>
                <CheckBox
                checkedColor='red'
...
<Text style={{color:'white',fontSize:15}}>اتصل</Text>
                ...
                <TouchableOpacity style={styles.buttonn} onPress={()=>{
                  Share.share({
               message: `تعرف على المختبر ${data.username} في تطبيق زيتا لاب
