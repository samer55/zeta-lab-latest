...
export default class Texts extends Component {
  ...
  componentDidMount() {
    ...
    firebaseApp.database().ref('text/'+'second').orderByChild('createdAt').limitToLast(this.state.counter).on('value',
    (snapshot) => {
      val = snapshot.val()
  ...
  _renderRow = (data) => {
    ...
    return (
      ...
      >
        ...
        <Text style={styles.info}><Text style={styles.bold}>{data.title}</Text><Text style={{color:'red'}}>{this.state.end}</Text>JD</Text>
