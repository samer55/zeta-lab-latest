...
export default class Amman extends Component {
  constructor(props) {
    super(props)
  ...
  componentDidMount() {
    ...
    firebaseApp.database().ref('amman').orderByChild('createdAt').limitToLast(this.state.counter).on('value',
    (snapshot) => {
  ...
  _renderRow = (data) => {
    ...
    return (
      ...
      >
        ...
        <Text style={{margin:3,fontSize:19,fontWeight:'bold',color:'#000000',fontFamily:'Roboto',selfAlign:'center',textAlign:'center'}}>{data.labn}</Text>
...
<Text style={{margin:3,fontSize:15,fontWeight:'bold',color:'#09090a',fontFamily:'Roboto',selfAlign:'center'}}>{data.title}</Text>
  ...
  _onEndReached = () => {
    ...
    if (!this.state.isEmpty && !this.state.isFinished && !this.state.isLoading) {
      ...
      firebaseApp.database().ref('amman').off()
      firebaseApp.database().ref('amman').orderByChild('createdAt').limitToLast(this.state.counter+1).on('value',
      (snapshot) => {
