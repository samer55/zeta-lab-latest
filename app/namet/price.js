...
export default class Com extends Component {
  ...
  componentDidMount() {
    ...
    firebaseApp.database().ref('posts').child(this.props.puid).limitToLast(this.state.counter).on('value',
    (snapshot) => {
  ...
  _onEndReached = () => {
    ...
    if (!this.state.isEmpty && !this.state.isFinished && !this.state.isLoading) {
      ...
      firebaseApp.database().ref('posts').child(this.props.puid).limitToLast(this.state.counter+1).on('value',
      (snapshot) => {
