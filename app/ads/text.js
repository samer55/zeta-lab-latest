...
export default class Texts extends Component {
  ...
  componentDidMount() {
    ...
    firebaseApp.database().ref('text').orderByChild('createdAt').limitToLast(this.state.counter).on('value',
    (snapshot) => {
      val = snapshot.val()
...
const p = data.puid
    ...
    snapshot.ref.remove();
      ...
      return (
        ...
        <TouchableOpacity  onPress={()=> this.user(data)}>
          ...
          <Text style={styles.info}><Text style={styles.bold}>Offer Price :</Text><Text style={{color:'red'}}>{data.price}</Text>JD</Text>
          <Text style={styles.info}><Text style={styles.bold}>Offer details :</Text><Text style={{color:'red'}}>{data.title}</Text>.</Text>
</TouchableOpacity>
  ...
  _onEndReached = () => {
    ...
    if (!this.state.isEmpty && !this.state.isFinished && !this.state.isLoading) {
      ...
      firebaseApp.database().ref('text').off()
      firebaseApp.database().ref('text').orderByChild('createdAt').limitToLast(this.state.counter+1).on('value',
      (snapshot) => {
