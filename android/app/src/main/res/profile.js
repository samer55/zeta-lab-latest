...
/>
   ...
   ima = () => {
     this.setState({
       postStatus: 'Posting...',
           ...
           const newPostKey = firebaseApp.database().ref('users').push().key
           const imageName = `${uid}.jpg`
           uploadImag(this.state.imagePat, imageName)
           .then(url => {
  ...
  firebaseApp.database().ref('users').child(this.props.appStore.user.uid).update({testn:this.state.testn,testp:this.state.testp})
  ...
  firebaseApp.database().ref('users').child(this.props.appStore.user.uid).update({info:this.state.info,time1:this.state.time1,time2:this.state.time2,phone:this.state.phone,week1:this.state.week1,week2:this.state.week2,home:this.state.isChecked})
...
<Text style={styles.info}>Time from:</Text>
   ...
   _renderButtoni = (text, onPress) => (
    ...
    _renderModalContenti = () => (
      <View style={styles.modalContent}>
      <View style={[styles.inputContainer, { marginBottom: 10 }]}>
        <TextInput
          style={styles.inputField}
          value={this.state.testn}
          onChangeText={(text) => this.setState({ testn: text })}
          autoCapitalize='words'
          autoCorrect={false}
          underlineColorAndroid='transparent'
        ...
        <View style={[styles.inputContainer, { marginBottom: 10 }]}>
          <TextInput
            style={styles.inputField}
            value={this.state.testp}
            onChangeText={(text) => this.setState({ testp: text })}
            autoCapitalize='words'
            autoCorrect={false}
            underlineColorAndroid='transparent'
            placeholder='prie'
      ...
      <TouchableOpacity onPress={this._handle}>
        <View style={styles.submitBtnContainer}>
          <Text style={styles.submitBtn}>{'SAVE'}</Text>
        </View>
      </TouchableOpacity>
...
<View style={{flexDirection:'row',justifyContent:'space-between'}}>
         ...
         <ListItem icon>
           <Left>
             <Button style={{ backgroundColor: "green" }}>
               <Icon onPress={this.pric} color='white' size={20} name="ios-map" />
             </Button>
           </Left>
           <Body>
             <Text>Prices</Text>
     ...
     return (
       <View style={styles.headerContainer}>
         ...
         <ImageBackground
           ...
           blurRadius={10}
           source={{
             uri: this.props.appStore.imagep,
           }}
         >
