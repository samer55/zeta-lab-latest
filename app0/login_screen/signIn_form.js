...
export default class SignInForm extends Component {
      ...
      <Animatable.View
        ...
        <View style={{flexDirection:'row',alignItems:'flex-start',justifyContent:'flex-start'}}>
          ...
          <View style={styles.submitBtnContainer}>
            <Text style={styles.submitBtn}>رجوع</Text>
          </View>
        </TouchableOpacity>
        </View>
          ...
          <View style={styles.btnContainers}>
            <TouchableOpacity onPress={this._handleForgotPassword}>
              <View style={styles.forgotBtnContainer}>
                <Text style={styles.forgotBtn}>{'نسيت كلمة السر؟'.toUpperCase()}</Text>
              </View>
            </TouchableOpacity>
            <TouchableOpacity onPress={this._handleSignIn}>
              <View style={styles.submitBtnContainer}>
                <Text style={styles.submitBtn}>{'دخول'.toUpperCase()}</Text>
              </View>
            </TouchableOpacity>
          </View>
  ...
  _handleSignIn = () => {
    this.setState({errMsg: 'جاري تسجيل'})
    if (this.state.email.length == 0) {
      this.setState({errMsg: "Please enter your email."})
    }
    else if (this.state.password.length == 0) {
      this.setState({errMsg: "ارجو ادخال كلمة السر"})
    }
    else {
    ...
    firebaseApp.database().ref('us').child(user.uid).once('value')
    .then((snapshot) => {
