...
const he = Dimensions.get('window').width
...
export default class Images extends Component {
  ...
  componentDidMount() {
    ...
    firebaseApp.database().ref('user_posts/'+ uid +'/image').orderByChild('createdAt').limitToLast(this.state.counter).on('value',
    (snapshot) => {
...
<TouchableOpacity  onPress={()=>   this.props.navigation.navigate('profilea', {name: data.labn, uid: data.uid})}>
        <ImageBackground
...
elevation: 24,color:'#FFFFFF',fontWeight:'800',fontSize:15}}>{data.labn}</Text>
<View style={{flexDirection:'row'}}>
  ...
  _onEndReached = () => {
    ...
    if (!this.state.isEmpty && !this.state.isFinished && !this.state.isLoading) {
      ...
      firebaseApp.database().ref('user_posts/'+ uid +'/image').off()
      firebaseApp.database().ref('user_posts/'+ uid +'/image').orderByChild('createdAt').limitToLast(this.state.counter+1).on('value',
      (snapshot) => {
